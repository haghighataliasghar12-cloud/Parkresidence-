...
async function initDB() {
  ...
  if (error || !data || data.length === 0) {
    for (const s of STAGES_DEFAULT) {
      await supabase.from(TABLE).upsert({
        id: s.id, name: s.name, description: s.desc,
        status: s.status, pct: s.pct, photos: s.photos
      });
    }
    stages = STAGES_DEFAULT.map(s => ({ ...s }));
  } else {
    stages = data.map(s => ({ ...s, desc: s.description || s.desc || "", photos: s.photos || [] }));
  }
  renderAll();
}
...
async function saveStage(stage) {
  await supabase.from(TABLE).upsert({
    id: stage.id, name: stage.name, description: stage.desc,
    status: stage.status, pct: stage.pct, photos: stage.photos
  });
}

// ─── STORAGE ──────────────────────────────────────────────
async function uploadPhoto(stageId, file) {
  const progEl = document.getElementById(`upload-prog-${stageId}`);
  const path = `stage-${stageId}/${Date.now()}_${file.name}`;

  if (progEl) progEl.textContent = "در حال آپلود...";

  const { error: upErr } = await supabase.storage.from(BUCKET).upload(path, file);
  if (upErr) {
    if (progEl) progEl.textContent = "خطا: " + upErr.message;
    return;
  }
  ...
  if (idx !== -1) {
    ...
    await saveStage(stages[idx]);
    renderAll();
    if (isAdmin) renderAdminPanel();
  }

  if (progEl) progEl.textContent = "آپلود شد ✓";
...
function calcTotal() {
  ...
  return Math.round(stages.reduce((a, s) => a + (s.pct || 0), 0) / stages.length);
}
...
function renderProgress() {
  ...
  document.getElementById("totalPct").textContent = toFa(total) + "٪";
  ...
  list.innerHTML = stages.map(s => {
    ...
    <div class="stage-card ${cardClass(s.status)}" id="stage-card-${s.id}">
      <div class="stage-header" data-id="${s.id}">
        <div class="stage-num ${numClass(s.status)}">${s.status === "done" ? "✓" : toFa(s.id)}</div>
  ...
  list.innerHTML = stages.map(s => {
    const thumbs = (s.photos || []).map(p =>
      `<div class="admin-thumb-wrap">
        ...
        <button class="btn-del-photo" data-id="${s.id}" data-path="${p.path}">✕</button>
      </div>`
    ).join("");
    ...
    <div class="admin-stage-item">
      <div class="admin-stage-top">
        <div class="admin-stage-name">${toFa(s.id)}. ${s.name}</div>
...
function initQR() {
  ...
  new QRCode(container, {
    ...
    colorDark: "#f0d080", colorLight: "#1a1d26",
...
// ─── EVENTS ───────────────────────────────────────────────
function bindEvents() {
  document.getElementById("btnQR").addEventListener("click", () => { initQR(); openModal("qrModal"); });
  ...
  document.getElementById("btnAdminOpen").addEventListener("click", () => {
    if (isAdmin) { renderAdminPanel(); openModal("adminPanel"); }
    else { document.getElementById("adminPass").value = ""; document.getElementById("adminError").textContent = ""; openModal("adminModal"); }
  });
